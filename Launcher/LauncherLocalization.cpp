	...
	size_t &translatedCount, size_t &totalCount, std::string &errorMessage)
{
	...
	output = "// Generated locally by Echelon from user-owned retail Russian resources.\r\n\r\n";
	for (size_t index = 0; index < lines.size();) {
		...
		if (openingQuote == std::string::npos || closingQuote == std::string::npos) {
			// Echelon @bugfix Codex 15/08/2026 Preserve intentionally empty STR entries used by HanPatch.
...
// Echelon @feature Codex 15/08/2026 Build a redistribution-safe Russian overlay from the user's retail CSF and installed mod strings.
