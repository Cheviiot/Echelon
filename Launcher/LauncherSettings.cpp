...
// Echelon @feature Codex 13/08/2026 Publish all profile files as one recoverable transaction.
...
std::string SerializeSagePatchOptions(const fs::path &path, const SagePatchOptions &options)
{
	...
	if (lines.empty()) {
		lines.push_back({"; Echelon safe QoL overrides", {}, {}});
	}
	if (!hasGameData) {
	...
	std::vector<std::string> &arguments, std::string &errorMessage)
{
	...
	// Echelon @feature Codex 13/08/2026 The shared engine parser supports quick start in both game branches.
