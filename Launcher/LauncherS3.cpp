...
Credentials LoadCredentials(const fs::path &modsRoot, const std::string &sourceId)
{
	...
	overrideFromEnvironment("ECHELON_S3_ACCESS_KEY", result.accessKey);
	overrideFromEnvironment("ECHELON_S3_SECRET_KEY", result.secretKey);
	overrideFromEnvironment("ECHELON_S3_SESSION_TOKEN", result.sessionToken);
	overrideFromEnvironment("ECHELON_S3_REGION", result.region);
	return result;
}
	...
	const std::string &expectedEtag = {})
{
	...
	curl_easy_setopt(curl, CURLOPT_USERAGENT, "Echelon/1 S3SigV4");
