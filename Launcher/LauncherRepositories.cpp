	...
	const std::atomic_bool *cancelRequested)
{
	...
	curl_easy_setopt(curl, CURLOPT_USERAGENT, "Echelon/1 RepositoryCatalogV1");
...
{
	// Echelon @feature Codex 14/08/2026 Keep retired repository configuration out of active source selection.
	if (fs::is_regular_file(modsRoot / "Repositories.ini")) {
		warnings.push_back("Repositories.ini is retired; only an explicit transport fixture may be configured");
	}
...
{
	// Echelon @refactor Codex 05/09/2026 Retain transport fixtures without a production catalog.
	if (const char *url = std::getenv("ECHELON_REPOSITORY_URL"); url && !HttpsOrigin(url).empty()) {
		return {{"fixture", {}, url, RepositoryFormat::ArsenalV1}};
	}
	return {};
}
	...
	const std::string &fallbackSourceId, const std::string &catalogUrl)
{
	...
	try {
		...
		for (const YAML::Node &item : items) {
			...
			// Echelon @feature Codex 15/08/2026 Preserve exact requirements and conflicts for real multi-branch ecosystems such as ROTR.
