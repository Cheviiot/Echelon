...
- Product PRs target `main` in `Cheviiot/Echelon`. Contributions to `fbraz3/GeneralsX` require a separately scoped upstream request.
- There is a subproject called `dxvk-macos` located under `GeneralsX/references/fbraz3-dxvk` folder, which is a fork of the original DXVK project. Commits related to that subproject should be made in that repository and follow the same commit message standards.
...
**Note**: For GeneralsX code changes, also see `AGENTS.md` for the code annotation standard (`// GeneralsX @keyword author DD/MM/YYYY Description`), which complements commit message discipline.