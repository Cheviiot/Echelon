...
3. Fetch both remotes and create `codex/upstream-sync-YYYY-MM-DD` from current `origin/main`.
4. Report the incoming commit range and touched subsystems before merging.
5. Merge `upstream/main` with `--no-ff --no-commit -Xsubtree=GeneralsX`; verify that upstream changes remain inside `GeneralsX/`.
