...
static bool s_echelonLauncherSession = false;
static bool s_echelonReturnRequested = false;
static uint32_t s_echelonTestReturnUpdates = 0;
static uint32_t s_echelonQuiescenceFlags = ECHELON_ENGINE_REQUIRED_QUIESCENCE_FLAGS;

void EchelonRequestReturnToLauncher()
{
	s_echelonReturnRequested = true;
}

bool EchelonIsLauncherSession()
{
	return s_echelonLauncherSession;
}

bool EchelonConsumeTestReturnRequest()
{
	if (s_echelonTestReturnUpdates == 0) {
		return false;
	}
	if (--s_echelonTestReturnUpdates != 0) {
		return false;
 ...
 */
#if !defined(ECHELON_ENGINE_MODULE)
...
#include "LauncherIntegration/EngineModuleEntry.inl"
