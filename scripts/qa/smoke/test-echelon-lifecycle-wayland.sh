#!/usr/bin/env bash
# Exercise repeated Echelon launcher/engine handoffs in an isolated Wayland compositor.
#
# Usage:
#   ./scripts/qa/smoke/test-echelon-lifecycle-wayland.sh [BUILD_PRESET] [CYCLES] [UPDATES] [START_PROFILE] [WINDOW_MODE]
...
qa_root="$(mktemp -d /tmp/echelon-lifecycle.XXXXXX)"
...
cp "${source_data_root}/Profiles/generals.ini" "${qa_home}/.Echelon/Profiles/"
...
qa_root="${qa_root}" launcher="${launcher}" dxvk_dir="${dxvk_dir}" cycle_count="${cycle_count}" \
	...
	mutter --headless --wayland --no-x11 --wayland-display=echelon-lifecycle-wl --virtual-monitor 1280x800 \
		>"${qa_root}/mutter.log" 2>&1 &
	compositor_pid=$!
	trap "kill ${compositor_pid} 2>/dev/null || true; wait ${compositor_pid} 2>/dev/null || true" EXIT
	for _ in $(seq 1 100); do
		[[ -S "${XDG_RUNTIME_DIR}/echelon-lifecycle-wl" ]] && break
		sleep 0.05
	done
	[[ -S "${XDG_RUNTIME_DIR}/echelon-lifecycle-wl" ]]
	env HOME="${qa_root}/home" WAYLAND_DISPLAY=echelon-lifecycle-wl SDL_VIDEODRIVER=wayland SDL_AUDIODRIVER=dummy \
...
# Echelon @bugfix Codex 13/08/2026 Verify the exact protocol lifetime that
...
if (( syncobj_created > 0 )); then
	...
	echo "ERROR: Explicit-sync was advertised but its surface lifetime was not exercised" >&2
	exit 1
else
