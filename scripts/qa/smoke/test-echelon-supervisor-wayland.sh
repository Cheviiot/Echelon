...
#
# Usage:
#   ./scripts/qa/smoke/test-echelon-supervisor-wayland.sh [BUILD_PRESET]
...
qa_root="$(mktemp -d /tmp/echelon-supervisor.XXXXXX)"
...
mkdir -p "${qa_runtime}" "${qa_home}/.Echelon/Profiles" \
	...
	"${qa_home}/.Echelon/Generals" "${qa_home}/.Echelon/GeneralsZH"
...
qa_root="${qa_root}" launcher="${launcher}" dxvk_dir="${dxvk_dir}" dbus-run-session -- bash -c '
	...
	for _ in $(seq 1 100); do
		[[ -S "${XDG_RUNTIME_DIR}/echelon-supervisor-wl" ]] && break
		sleep 0.05
	done
	[[ -S "${XDG_RUNTIME_DIR}/echelon-supervisor-wl" ]]
	env HOME="${qa_root}/home" WAYLAND_DISPLAY=echelon-supervisor-wl SDL_VIDEODRIVER=wayland SDL_AUDIODRIVER=dummy \
