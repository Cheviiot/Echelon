...
#
# Usage:
#   ./scripts/qa/smoke/test-generals-arsenal-mod-manager-wayland.sh [BUILD_PRESET]
...
qa_root="$(mktemp -d /tmp/generals-arsenal-mod-manager.XXXXXX)"
qa_home="${qa_root}/home"
mods_root="${qa_home}/.GeneralsArsenal/Mods"
mkdir -p "${qa_root}/runtime" "${qa_home}/.GeneralsArsenal/Launcher" \
...
make_fixture() {
	...
	cp "${repo_root}/assets/launcher/generals-arsenal-logo.png" "${root}/cover.png"
	printf '%s\n' \
		'[Modification]' 'SchemaVersion=2' "Id=${id}" 'SourceId=qa' "Name=${name}" "Version=${version}" \
		'Engine=generals' "Type=${type}" "ParentId=${parent}" 'Source=https://example.invalid/arsenal-qa' \
		'RootPath=content' 'CoverImage=cover.png' 'SHA256=' "ContentFingerprint=${fingerprint}" \
		>"${root}/manifest.ini"
}

make_fixture mod qa:desert 'Desert Arsenal' '' 1.0
make_fixture mod qa:desert 'Desert Arsenal' '' 0.9
...
printf '%s\n' \
	'[Source qa]' 'Id=qa' 'Engine=generals' 'URL=https://example.invalid/catalog.yaml' 'Format=arsenal-v1' \
	>"${mods_root}/Repositories.ini"
printf '%s\n' \
	'SchemaVersion: 1' 'SourceId: qa' 'Items:' \
	'  - Id: desert' '    Engine: generals' '    Type: mod' '    Name: Desert Arsenal' '    Version: "2.0"' \
...
QA_ROOT="${qa_root}" LAUNCHER="${launcher}" dbus-run-session -- bash -c '
	...
	mutter --headless --wayland --no-x11 --wayland-display=arsenal-mods-wl --virtual-monitor 1920x1080 \
		>"${QA_ROOT}/mutter.log" 2>&1 &
	compositor_pid=$!
	trap "kill ${compositor_pid} 2>/dev/null || true; wait ${compositor_pid} 2>/dev/null || true" EXIT
	for _ in $(seq 1 100); do
		[[ -S "${XDG_RUNTIME_DIR}/arsenal-mods-wl" ]] && break
		sleep 0.05
	done
	[[ -S "${XDG_RUNTIME_DIR}/arsenal-mods-wl" ]]
	for page in mods patches addons; do
		env HOME="${QA_ROOT}/home" WAYLAND_DISPLAY=arsenal-mods-wl SDL_VIDEODRIVER=wayland SDL_AUDIODRIVER=dummy \
			DBUS_SESSION_BUS_ADDRESS=unix:path=/dev/null timeout 45s "${LAUNCHER}" --internal-ui-worker --mods \
