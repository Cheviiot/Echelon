#!/usr/bin/env bash
# Exercise the installed Generals: Arsenal Flatpak in a private Wayland compositor and HOME.
#
# Usage:
#   ./scripts/qa/smoke/test-generals-arsenal-flatpak-wayland.sh [CYCLES] [UPDATES] [WINDOW_MODE]
...
qa_root="$(mktemp -d /tmp/generals-arsenal-flatpak.XXXXXX)"
...
qa_root="${qa_root}" qa_home="${qa_home}" app_id="${app_id}" cycle_count="${cycle_count}" \
	...
	mutter --headless --wayland --no-x11 --wayland-display=arsenal-flatpak-wl --virtual-monitor 1280x800 \
		>"${qa_root}/mutter.log" 2>&1 &
	compositor_pid=$!
	trap "kill ${compositor_pid} 2>/dev/null || true; wait ${compositor_pid} 2>/dev/null || true" EXIT
	for _ in $(seq 1 100); do
		[[ -S "${XDG_RUNTIME_DIR}/arsenal-flatpak-wl" ]] && break
		sleep 0.05
	done
	[[ -S "${XDG_RUNTIME_DIR}/arsenal-flatpak-wl" ]]
	env WAYLAND_DISPLAY=arsenal-flatpak-wl SDL_VIDEODRIVER=wayland SDL_AUDIODRIVER=dummy \
...
if ! rg -q 'launcher font: .*/share/generals-arsenal/DejaVuSans.ttf' "${log_file}"; then
...
# GeneralsArsenal @bugfix Codex 13/08/2026 Ensure the packaged DXVK instance
