...
#
# Usage:
#   ./scripts/qa/smoke/test-echelon-download-resume.sh [BUILD_PRESET]
...
test_binary="${repo_root}/build/${build_preset}/Launcher/echelon_settings_tests"
server_script="${repo_root}/scripts/qa/fixtures/echelon_https_range_server.py"
...
qa_root="$(mktemp -d /tmp/echelon-download-resume.XXXXXX)"
...
env \
	...
	ECHELON_QA_CA_BUNDLE="${certificate}" \
	ECHELON_QA_HTTPS_URL="https://127.0.0.1:${port}" \
	ECHELON_QA_PACKAGE_PATH="${package}" \
	ECHELON_QA_PACKAGE_SHA256="${package_sha256}" \
	ECHELON_QA_PACKAGE_SIZE="${package_size}" \
...
if ! rg -q '^PASS: Echelon settings persistence$' "${qa_root}/test.log"; then
