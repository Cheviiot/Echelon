...
static bool GeneralsArsenalModuleInitialize(const GeneralsArsenalEngineHostV2 *host)
{
	...
	GeneralsArsenalModuleLog(host, "INFO: Zero Hour engine module initialized");
	return true;
}
...
static GeneralsArsenalEngineResultV2 GeneralsArsenalModuleRun(const GeneralsArsenalEngineHostV2 *host)
{
	...
	try {
		GeneralsArsenalModulePhase(host, GENERALS_ARSENAL_ENGINE_PHASE_STARTING, "Zero Hour session is starting");
		...
		if (!GeneralsArsenalContentRuntime::Configure(host->content_layers, host->content_layer_count, contentError)) {
			...
			GeneralsArsenalModulePhase(host, GENERALS_ARSENAL_ENGINE_PHASE_FAILED, "Zero Hour content stack validation failed");
			return GENERALS_ARSENAL_ENGINE_FATAL_ERROR;
		}
		if (!GeneralsArsenalModuleInitialize(host)) {
			GeneralsArsenalContentRuntime::Clear();
			GeneralsArsenalModulePhase(host, GENERALS_ARSENAL_ENGINE_PHASE_FAILED, "Zero Hour module initialization failed");
			return GENERALS_ARSENAL_ENGINE_FATAL_ERROR;
		}
		...
		CommandLine::parseCommandLineForStartup();
		GeneralsArsenalModulePhase(host, GENERALS_ARSENAL_ENGINE_PHASE_RUNNING, "Zero Hour session is running");
		const Int exitCode = GameMain();
		GeneralsArsenalModulePhase(host, GENERALS_ARSENAL_ENGINE_PHASE_STOPPING, "Zero Hour session is stopping");
		...
		if ((s_generalsArsenalQuiescenceFlags & GENERALS_ARSENAL_ENGINE_REQUIRED_QUIESCENCE_FLAGS) !=
			...
			GeneralsArsenalModuleLog(host, "ERROR: Zero Hour engine did not reach a quiescent state");
			GeneralsArsenalModulePhase(host, GENERALS_ARSENAL_ENGINE_PHASE_FAILED, "Zero Hour teardown is incomplete");
			return GENERALS_ARSENAL_ENGINE_FATAL_ERROR;
		}
		GeneralsArsenalModulePhase(host, GENERALS_ARSENAL_ENGINE_PHASE_QUIESCENT, "Zero Hour session is quiescent");

		if (exitCode != 0) {
			GeneralsArsenalModuleLog(host, "ERROR: Zero Hour engine session failed");
			return GENERALS_ARSENAL_ENGINE_FATAL_ERROR;
		}
		...
		fprintf(stderr, "FATAL: Zero Hour module exception: %s\n", error.what());
		fflush(stderr);
	} catch (...) {
		GeneralsArsenalModuleLog(host, "FATAL: Unknown Zero Hour module exception");
	}
	...
	GeneralsArsenalContentRuntime::Clear();
	s_generalsArsenalQuiescenceFlags = GeneralsArsenalCollectQuiescenceFlags();
	GeneralsArsenalModulePhase(host, GENERALS_ARSENAL_ENGINE_PHASE_FAILED, "Zero Hour session failed");
	return GENERALS_ARSENAL_ENGINE_FATAL_ERROR;
}
