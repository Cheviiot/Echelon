...
bool ValidateBigArchive(const fs::path &path, std::string &errorMessage)
{
	...
	if (trailerSize != 0) {
		// GeneralsArsenal @bugfix Codex 15/08/2026 Accept the two directory suffixes produced by deployed SAGE BIG tools.
	...
	std::string &errorMessage, ModificationOperationProgress *progress)
{
	...
	for (const fs::path &file : files) {
		...
		// GeneralsArsenal @feature Codex 15/08/2026 Treat GenLauncher .gib payloads as immutable BIG archives without retail-file renaming.
	...
	indexText = index.str();
	// GeneralsArsenal @bugfix Codex 14/08/2026 Verification must work for a genuinely read-only installed tree.
	...
	ModificationOperationProgress *progress)
{
	...
	if (inputIsDirectory) {
		...
		// GeneralsArsenal @bugfix Codex 15/08/2026 Import standalone GenLauncher .gib payloads through the same validated BIG path.
