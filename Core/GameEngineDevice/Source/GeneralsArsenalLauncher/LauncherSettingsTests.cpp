...
using namespace GeneralsArsenalLauncher;
...
// GeneralsArsenal @feature Codex 13/08/2026 Exercise settings persistence independently from the graphical launcher.
int main()
{
	const auto unique = std::chrono::steady_clock::now().time_since_epoch().count();
	const fs::path testRoot = fs::temp_directory_path() / ("generals-arsenal-settings-" + std::to_string(unique));
	...
	// GeneralsArsenal @feature Codex 13/08/2026 Validate safe manifest discovery and exact archive filtering.
	...
	// GeneralsArsenal @test Codex 14/08/2026 Lock both external YAML compatibility and the stricter Arsenal schema.
	...
	// GeneralsArsenal @test Codex 15/08/2026 Prove that one catalog can add a complete content stack without launcher changes.
	...
	const std::vector<RepositorySource> builtInSources = BuiltInRepositorySources();
	Check(builtInSources.size() == 1 && builtInSources[0].id == "arsenal" &&
		builtInSources[0].format == RepositoryFormat::ArsenalV1 && builtInSources[0].engine.empty(),
		"normal launcher sessions must use only the centralized Arsenal catalog");
...
#if defined(_WIN32)
	_putenv_s("GENERALS_ARSENAL_S3_ACCESS_KEY", "test-access");
	_putenv_s("GENERALS_ARSENAL_S3_SECRET_KEY", "test-secret");
#else
	setenv("GENERALS_ARSENAL_S3_ACCESS_KEY", "test-access", 1);
	setenv("GENERALS_ARSENAL_S3_SECRET_KEY", "test-secret", 1);
#endif
...
#if defined(_WIN32)
	_putenv_s("GENERALS_ARSENAL_S3_ACCESS_KEY", "");
	_putenv_s("GENERALS_ARSENAL_S3_SECRET_KEY", "");
#else
	unsetenv("GENERALS_ARSENAL_S3_ACCESS_KEY");
	unsetenv("GENERALS_ARSENAL_S3_SECRET_KEY");
#endif
	if (const char *networkUrl = std::getenv("GENERALS_ARSENAL_QA_HTTPS_URL"); networkUrl && networkUrl[0]) {
		const char *packagePathValue = std::getenv("GENERALS_ARSENAL_QA_PACKAGE_PATH");
		const char *packageSha256 = std::getenv("GENERALS_ARSENAL_QA_PACKAGE_SHA256");
		const char *packageSizeValue = std::getenv("GENERALS_ARSENAL_QA_PACKAGE_SIZE");
...
#if defined(_WIN32)
	_putenv_s("GENERALS_ARSENAL_DISABLED_BIG_FILES", "00Russian.big; 00RussianZH.big");
#else
	setenv("GENERALS_ARSENAL_DISABLED_BIG_FILES", "00Russian.big; 00RussianZH.big", 1);
#endif
	Check(GeneralsArsenalArchivePolicy::IsArchiveDisabled("/game/00RUSSIAN.big") &&
		GeneralsArsenalArchivePolicy::IsArchiveDisabled("C:\\game\\00RussianZH.big") &&
		!GeneralsArsenalArchivePolicy::IsArchiveDisabled("00RussianPatch.big"),
		"archive policy must match exact basenames and never prefixes");
#if defined(_WIN32)
	_putenv_s("GENERALS_ARSENAL_DISABLED_BIG_FILES", "");
#else
	unsetenv("GENERALS_ARSENAL_DISABLED_BIG_FILES");
#endif
	...
	const GeneralsArsenalContentLayerV1 runtimeLayers[] = {
		{sizeof(GeneralsArsenalContentLayerV1), GENERALS_ARSENAL_CONTENT_LAYER_MOD,
			"fixture:mod", "1.0", modRootString.c_str(), 100, modFingerprint.c_str()},
		{sizeof(GeneralsArsenalContentLayerV1), GENERALS_ARSENAL_CONTENT_LAYER_PATCH,
			"fixture:patch", "1.0", patchRootString.c_str(), 200, patchFingerprint.c_str()}
	};
	message.clear();
	Check(GeneralsArsenalContentRuntime::Configure(runtimeLayers, 2, message),
		"ABI content layers must validate before activation");
	fs::path resolvedLayerFile;
	Check(GeneralsArsenalContentRuntime::ResolveReadPath("Data\\INI\\Layer.ini", resolvedLayerFile) &&
	...
	const std::vector<fs::path> layeredFiles =
		GeneralsArsenalContentRuntime::ListFiles("Data/INI", "*.ini", true);
	...
	Check(GeneralsArsenalContentRuntime::IsClear(), "content layer runtime must be empty at session quiescence");
	...
	// GeneralsArsenal @test Codex 15/08/2026 Generate a Russian overlay from retail CSF data without redistributing it.
	...
	// GeneralsArsenal @test Codex 15/08/2026 Allow an opt-in audit of the complete official ROTR file set before catalog publication.
	if (const char *rotrPath = std::getenv("GENERALS_ARSENAL_QA_ROTR_PATH"); rotrPath && rotrPath[0]) {
	...
	std::puts("PASS: Generals: Arsenal settings persistence");
	return 0;
}
