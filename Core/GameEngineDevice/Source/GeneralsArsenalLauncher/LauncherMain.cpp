...
void SDLCALL LauncherSDLLogOutput(void *userdata, int category, SDL_LogPriority priority, const char *message)
{
	if (category == SDL_LOG_CATEGORY_VIDEO && message && std::strstr(message, "Wayland display connection closed")) {
		...
		fflush(stderr);
		// GeneralsArsenal @bugfix Codex 13/08/2026 Never let SDL's synthetic quit event disguise a poisoned Wayland connection as a clean user exit.
...
struct LoadedModule
{
	...
	const GeneralsArsenalEngineModuleV2 *api = nullptr;
};
...
LauncherPaths BuildLauncherPaths()
{
	...
	const fs::path root = home / GeneralsArsenalBrand::kDataDirectory;
	return {
		home,
		root,
		root / "Launcher",
...
void FilterSoftwareVulkanICDs()
{
	...
	fprintf(stderr, "INFO: Generals: Arsenal Vulkan ICD filter: %s\n", driverList.c_str());
	fflush(stderr);
#endif
}
...
public:
	...
	{
		reset();
		const char *overrideFont = std::getenv("GENERALS_ARSENAL_LAUNCHER_FONT");
		...
		// GeneralsArsenal @bugfix Codex 12/08/2026 Prefer the packaged launcher font and cover Freedesktop runtime layouts.
		candidates.insert(candidates.end(), {
			executableDirectory / GENERALS_ARSENAL_LAUNCHER_FONT_FILE,
			executableDirectory / ".." / "share" / GeneralsArsenalBrand::kInstallDataDirectory /
				GENERALS_ARSENAL_LAUNCHER_FONT_FILE,
			executableDirectory / ".." / "Resources" / GENERALS_ARSENAL_LAUNCHER_FONT_FILE,
		...
		fprintf(stderr, "INFO: Generals: Arsenal launcher font: %s\n", selected.string().c_str());
		fflush(stderr);
...
// GeneralsArsenal @refactor Codex 13/08/2026 Keep the launcher's independent emerald HUD identity in one palette.
...
// GeneralsArsenal @feature Codex 13/08/2026 Give launcher and per-engine settings a full-size SAGE-styled workspace.
...
// GeneralsArsenal @feature Codex 14/08/2026 Reserve one full logical workspace for the native modification manager.
struct ModsLayout
{
	...
	SDL_FRect refreshButton{120.0f, 820.0f, 155.0f, 54.0f};
	SDL_FRect importButton{285.0f, 820.0f, 225.0f, 54.0f};
	SDL_FRect importFolderButton{520.0f, 820.0f, 180.0f, 54.0f};
...
{
	const GeneralsArsenalLauncher::InstalledModification *installed = nullptr;
	const GeneralsArsenalLauncher::RepositoryModification *available = nullptr;

	std::string id() const { return installed ? installed->id : available->id; }
	std::string name() const { return installed ? installed->name : available->name; }
	std::string version() const { return available ? available->version : installed->version; }
	std::string sourceId() const { return installed ? installed->sourceId : available->sourceId; }
	GeneralsArsenalLauncher::ModificationType type() const
	{
		return installed ? installed->type : available->type;
	}
	const std::vector<std::string> &requirements() const
	{
		return installed ? installed->requirements : available->requirements;
	}
	const std::vector<std::string> &conflicts() const
	{
		return installed ? installed->conflicts : available->conflicts;
	}
	bool hasUpdate() const
	{
		return installed && available &&
			GeneralsArsenalLauncher::CompareModificationVersions(installed->version, available->version) < 0;
	}
	fs::path coverPath() const
	{
		if (installed && !installed->coverImagePath.empty()) return installed->coverImagePath;
		return available ? available->coverCachePath : fs::path{};
	}
};
...
public:
	bool initialize(SDL_Renderer *renderer)
	{
		reset();
		std::vector<fs::path> candidates;
		if (const char *overridePath = std::getenv("GENERALS_ARSENAL_LAUNCHER_BACKGROUND"); overridePath && overridePath[0]) {
			candidates.emplace_back(overridePath);
		}
		const char *basePath = SDL_GetBasePath();
		const fs::path executableDirectory = basePath && basePath[0] ? fs::path(basePath) : fs::current_path();
		candidates.insert(candidates.end(), {
			executableDirectory / GENERALS_ARSENAL_LAUNCHER_BACKGROUND_FILE,
			executableDirectory / ".." / "share" / GeneralsArsenalBrand::kInstallDataDirectory /
				GENERALS_ARSENAL_LAUNCHER_BACKGROUND_FILE,
			executableDirectory / ".." / "Resources" / GENERALS_ARSENAL_LAUNCHER_BACKGROUND_FILE
		});
		for (const fs::path &candidate : candidates) {
...
// GeneralsArsenal @feature Codex 12/08/2026 Render the original Arsenal wordmark with aspect-preserving scaling.
class LauncherLogo
{
public:
	bool initialize(SDL_Renderer *renderer)
	{
		reset();
		std::vector<fs::path> candidates;
		if (const char *overridePath = std::getenv("GENERALS_ARSENAL_LAUNCHER_LOGO"); overridePath && overridePath[0]) {
			candidates.emplace_back(overridePath);
		}
		const char *basePath = SDL_GetBasePath();
		const fs::path executableDirectory = basePath && basePath[0] ? fs::path(basePath) : fs::current_path();
		candidates.insert(candidates.end(), {
			executableDirectory / GENERALS_ARSENAL_LAUNCHER_LOGO_FILE,
			executableDirectory / ".." / "share" / GeneralsArsenalBrand::kInstallDataDirectory /
				GENERALS_ARSENAL_LAUNCHER_LOGO_FILE,
			executableDirectory / ".." / "Resources" / GENERALS_ARSENAL_LAUNCHER_LOGO_FILE
		});
		for (const fs::path &candidate : candidates) {
			m_texture = IMG_LoadTexture(renderer, candidate.string().c_str());
...
// GeneralsArsenal @feature Codex 14/08/2026 Cache the exact color and grayscale card art variants used by the mod catalog.
class ModCoverCache
{
public:
...
bool LauncherUsesRussian(const GeneralsArsenalLauncher::LauncherSettings &settings)
{
	if (settings.language == GeneralsArsenalLauncher::LanguageMode::Russian) return true;
	if (settings.language == GeneralsArsenalLauncher::LanguageMode::English) return false;
...
void DrawModificationCard(SDL_Renderer *renderer, LauncherFont &font, ModCoverCache &covers,
	...
	int activeOrder, bool russian, bool updateFlash)
{
	DrawFilledRect(renderer, card, selected ? LauncherPalette::kControlHilite : LauncherPalette::kControl);
	DrawOutline(renderer, card, selected || updateFlash ? LauncherPalette::kAccentBright : LauncherPalette::kAccentMuted,
		selected || updateFlash ? 3 : 1);
	...
	std::string status = modification.hasUpdate() ? Localized(russian, "UPDATE AVAILABLE", "ДОСТУПНО ОБНОВЛЕНИЕ") :
			(modification.installed ? Localized(russian, "INSTALLED", "УСТАНОВЛЕН") :
				Localized(russian, "AVAILABLE", "ДОСТУПЕН"));
...
void DrawLauncherBranding(SDL_Renderer *renderer, LauncherFont &font, const LauncherLogo &logo)
{
	...
	font.draw("GENERALS", centerX, 100.0f, 30.0f, LauncherPalette::kText, true);
	font.draw("ARSENAL", centerX, 157.0f, 67.0f, LauncherPalette::kAccentBright, true);
}
...
{
	return engine == kGeneralsProfileId ? GENERALS_ARSENAL_GENERALS_MODULE_FILE : GENERALS_ARSENAL_ZEROHOUR_MODULE_FILE;
}

fs::path ExecutableDirectory()
{
	const char *basePath = SDL_GetBasePath();
	return basePath && basePath[0] ? fs::path(basePath) : fs::current_path();
}
...
LoadedModule *LoadModule(const std::string &engine, std::unordered_map<std::string, LoadedModule> &modules, std::string &errorMessage)
{
	...
	std::vector<fs::path> candidates = {
		...
		ExecutableDirectory() / ".." / "lib" / GeneralsArsenalBrand::kInstallDataDirectory / ModuleFileName(engine)
	};
	SDL_SharedObject *handle = nullptr;
	for (const fs::path &candidate : candidates) {
	...
	const SDL_FunctionPointer function = SDL_LoadFunction(handle, GeneralsArsenalBrand::kModuleExport);
	if (!function) {
		errorMessage = "Engine module has no GeneralsArsenal_GetEngineModuleV2 export";
		return nullptr;
	}
	auto getModule = reinterpret_cast<GeneralsArsenalGetEngineModuleV2Fn>(function);
	const GeneralsArsenalEngineModuleV2 *api = getModule();
	if (!api || api->struct_size < sizeof(GeneralsArsenalEngineModuleV2) || api->abi_version != GENERALS_ARSENAL_ENGINE_ABI_VERSION ||
...
void EnginePhase(void *, GeneralsArsenalEnginePhaseV2 phase, const char *message)
{
...
bool CanControlWindowPosition()
{
	...
	// GeneralsArsenal @bugfix Codex 14/08/2026 Wayland intentionally gives placement authority to the compositor.
	return !driver || std::strcmp(driver, "wayland") != 0;
}

// GeneralsArsenal @feature Codex 14/08/2026 Centralize every launcher/engine presentation transition under host ownership.
...
// GeneralsArsenal @test Codex 14/08/2026 Make the host-owned window invariant observable in automated lifecycle tests.
...
GeneralsArsenalEngineResultV2 RunProfile(LauncherProfile &profile, SDL_Window *window, const std::vector<std::string> &gameArguments,
	const GeneralsArsenalLauncher::LauncherSettings &launcherSettings,
	...
	const GeneralsArsenalLauncher::ModificationStack *contentStack = nullptr)
{
	quiescenceFlags = 0;
	std::vector<std::string> arguments;
	arguments.emplace_back(GeneralsArsenalBrand::kProductSlug);
	...
	if (contentStack && !contentStack->layers.empty()) {
		...
		// GeneralsArsenal @bugfix Codex 14/08/2026 Rebuild every content fingerprint immediately before handing paths to the engine.
		...
		for (const GeneralsArsenalLauncher::InstalledModification *layer : contentStack->layers) {
			if (!layer || !GeneralsArsenalLauncher::VerifyInstalledModification(*layer, errorMessage)) {
	...
	if (!module) return GENERALS_ARSENAL_ENGINE_FATAL_ERROR;
	const GeneralsArsenalLauncher::ProfileLaunchSettings &profileSettings =
		GeneralsArsenalLauncher::SettingsForProfile(launcherSettings, profile.id);
	if (!headless) {
		if (!GeneralsArsenalLauncher::AppendProfileArguments(profileSettings, arguments, errorMessage)) {
			return GENERALS_ARSENAL_ENGINE_FATAL_ERROR;
		}
	}
	// GeneralsArsenal @refactor Codex 14/08/2026 Vanilla profile buttons never inherit a launcher-selected modification.
	// Explicit legacy -mod arguments remain untouched for direct and automated launches.
	// GeneralsArsenal @feature Codex 14/08/2026 Resolve the final command-line display intent before DXVK creates its swapchain.
	if (!headless && windowCoordinator) {
		std::string optionsError;
		const GeneralsArsenalLauncher::GameOptions options =
			GeneralsArsenalLauncher::LoadGameOptions(profile.userDataRoot / "Options.ini", optionsError);
	...
	SetEnvironment("GENERALS_ARSENAL_USER_DATA_ROOT", profile.userDataRoot.string());
	const bool useRussianLocalization = headless || profileSettings.russianLocalization;
	if (!useRussianLocalization) {
		SetEnvironment("GENERALS_ARSENAL_DISABLED_BIG_FILES", profile.engine == kZeroHourProfileId ?
			"00Russia.big;00Russian.big;00RussiaZH.big;00RussianZH.big" : "00Russia.big;00Russian.big");
	} else {
		SetEnvironment("GENERALS_ARSENAL_DISABLED_BIG_FILES", "");
	}
	SetEnvironment("GENERALS_ARSENAL_UI_LANGUAGE",
	...
	GeneralsArsenalEngineHostV2 host{};
	host.struct_size = sizeof(host);
	host.abi_version = GENERALS_ARSENAL_ENGINE_ABI_VERSION;
	...
	// GeneralsArsenal @feature Codex 14/08/2026 The launcher retains physical window ownership across every in-process engine session.
	host.window_policy = headless ? GENERALS_ARSENAL_ENGINE_WINDOW_POLICY_ENGINE_OWNED :
		GENERALS_ARSENAL_ENGINE_WINDOW_POLICY_HOST_OWNED;
	...
	std::vector<GeneralsArsenalContentLayerV1> hostContentLayers;
	if (contentStack) {
		...
		for (const GeneralsArsenalLauncher::InstalledModification *layer : contentStack->layers) {
			...
			if (layer->type == GeneralsArsenalLauncher::ModificationType::Patch) type = GENERALS_ARSENAL_CONTENT_LAYER_PATCH;
			else if (layer->type == GeneralsArsenalLauncher::ModificationType::Addon) type = GENERALS_ARSENAL_CONTENT_LAYER_ADDON;
			hostContentLayers.push_back(GeneralsArsenalContentLayerV1{sizeof(GeneralsArsenalContentLayerV1), type,
	...
	const GeneralsArsenalEngineResultV2 result = module->api->run(&host);
	GeneralsArsenalEngineQuiescenceReportV2 report{};
	report.struct_size = sizeof(report);
	quiescenceFlags = module->api->query_quiescence(&report);
	fprintf(stderr, "INFO: Generals: Arsenal engine quiescence flags: 0x%08x\n", quiescenceFlags);
	fflush(stderr);
...
struct ParsedArguments
{
	...
	bool internalTestWaitForRepository = false;
	std::string internalTestInstallModification;
...
{
	const fs::path executable = ExecutableDirectory() / GeneralsArsenalBrand::kProductSlug;
	...
	for (;;) {
		...
		if (!worker) {
			fprintf(stderr, "FATAL: Cannot start the Generals: Arsenal UI worker: %s\n", SDL_GetError());
			return 1;
		}
		const Sint64 workerPid = SDL_GetNumberProperty(
			SDL_GetProcessProperties(worker), SDL_PROP_PROCESS_PID_NUMBER, -1);
		fprintf(stderr, "INFO: Generals: Arsenal supervisor started UI worker pid=%lld recovered=%d\n",
		...
		if (!waited) {
			fprintf(stderr, "FATAL: Cannot wait for the Generals: Arsenal UI worker: %s\n", SDL_GetError());
			return 1;
		}
		...
		fprintf(stderr, "WARNING: Generals: Arsenal UI worker exited with code %d; restarting the launcher (%zu/%d)\n",
			exitCode, recoveries.size(), kMaximumRecoveriesPerMinute);
		fflush(stderr);
		if (recoveries.size() > static_cast<size_t>(kMaximumRecoveriesPerMinute)) {
			fprintf(stderr, "FATAL: Generals: Arsenal recovery stopped after repeated UI worker failures\n");
...
ParsedArguments ParseArguments(int argc, char **argv)
{
	...
	for (int index = 1; index < argc; ++index) {
		...
		} else if (argument == "--internal-test-wait-for-repository") {
			parsed.internalTestWaitForRepository = true;
		} else if (argument.compare(0, 37, "--internal-test-install-modification=") == 0) {
			parsed.internalTestInstallModification = ToLower(argument.substr(37));
...
int main(int argc, char **argv)
{
	...
	// GeneralsArsenal @feature Codex 13/08/2026 Keep a windowless parent alive so a fatal Wayland/Vulkan worker failure is recoverable.
	...
	const GeneralsArsenalLauncher::ModificationRecoverySummary modificationRecovery =
		GeneralsArsenalLauncher::RecoverInterruptedModificationOperations(paths.mods);
	...
	GeneralsArsenalLauncher::ModificationCatalog modificationCatalog =
		GeneralsArsenalLauncher::LoadModificationCatalog(paths.mods);
	GeneralsArsenalLauncher::RepositoryCatalog repositoryCatalog =
		GeneralsArsenalLauncher::LoadCachedRepositoryCatalog(paths.mods / "Cache");
	std::string modificationProfileWarning;
	GeneralsArsenalLauncher::ModificationSelectionProfileV1 generalsModificationProfile =
		GeneralsArsenalLauncher::LoadModificationSelectionProfile(paths.mods, kGeneralsProfileId, modificationProfileWarning);
	if (!modificationProfileWarning.empty()) {
		fprintf(stderr, "WARNING: %s\n", modificationProfileWarning.c_str());
		modificationProfileWarning.clear();
	}
	GeneralsArsenalLauncher::ModificationSelectionProfileV1 zeroHourModificationProfile =
		GeneralsArsenalLauncher::LoadModificationSelectionProfile(paths.mods, kZeroHourProfileId, modificationProfileWarning);
	if (!modificationProfileWarning.empty()) fprintf(stderr, "WARNING: %s\n", modificationProfileWarning.c_str());
	for (const std::string &warning : modificationCatalog.warnings) {
		fprintf(stderr, "WARNING: %s\n", warning.c_str());
	}
	std::unordered_map<std::string, LoadedModule> modules;
	std::optional<GeneralsArsenalLauncher::ModificationStack> commandLineStack;
	...
	if (hasExplicitStack) {
		...
		GeneralsArsenalLauncher::ModificationStack stack;
		std::string stackError;
		if (!GeneralsArsenalLauncher::ResolveModificationStack(modificationCatalog, parsed.profile,
	...
	if (!GeneralsArsenalLauncher::RecoverInterruptedSettingsBundle(paths.root, settingsLoadError)) {
		fprintf(stderr, "WARNING: %s\n", settingsLoadError.c_str());
		fflush(stderr);
	}
	settingsLoadError.clear();
	GeneralsArsenalLauncher::LauncherSettings launcherSettings =
		GeneralsArsenalLauncher::LoadLauncherSettings(paths.settings, settingsLoadError);
	...
	if (parsed.headless) {
		...
		const GeneralsArsenalEngineResultV2 result = RunProfile(
		...
		ExitWithoutGlobalDestructors(result == GENERALS_ARSENAL_ENGINE_FATAL_ERROR ? 1 : 0);
	}
	...
	if (!testWindowOverride && launcherSettings.windowMode == GeneralsArsenalLauncher::LauncherWindowMode::Fullscreen) {
	...
	SDL_Window *window = SDL_CreateWindow(GeneralsArsenalBrand::kProductName, initialWindowWidth, initialWindowHeight, windowFlags);
	if (!window) {
	...
	fprintf(stderr, "INFO: Generals: Arsenal launcher display mode=%s size=%dx%d\n",
	...
	auto createRenderer = [&]() -> bool {
		// GeneralsArsenal @bugfix Codex 13/08/2026 Use one explicit backend and validate its first present at every handoff.
		renderer = SDL_CreateRenderer(window, "vulkan");
		if (!renderer) return false;
		fprintf(stderr, "INFO: Generals: Arsenal launcher renderer: %s\n", SDL_GetRendererName(renderer));
		fflush(stderr);
		...
		if (!font.initialize(renderer)) {
			fprintf(stderr, "WARNING: Generals: Arsenal launcher font initialization failed\n");
			fflush(stderr);
		}
	...
	if (parsed.internalTestSupervisorRecovery && !parsed.internalRecovered) {
		fprintf(stderr, "INFO: Generals: Arsenal supervisor test is terminating the initial UI worker abruptly\n");
		fflush(stderr);
		std::abort();
	}
	...
	GeneralsArsenalLauncher::LauncherSettings settingsDraft = launcherSettings;
	GeneralsArsenalLauncher::GameOptions generalsOptionsDraft;
	GeneralsArsenalLauncher::GameOptions zeroHourOptionsDraft;
	GeneralsArsenalLauncher::SagePatchOptions generalsSagePatchDraft;
	GeneralsArsenalLauncher::SagePatchOptions zeroHourSagePatchDraft;
	...
	bool internalTestInstallStarted = false;
	bool internalTestInstallFinished = false;
	std::future<MigrationResult> migration;
	bool migrationRunning = false;
	std::future<GeneralsArsenalLauncher::RepositoryCatalog> repositoryRefresh;
	bool repositoryRefreshRunning = false;
	std::shared_ptr<std::atomic_bool> repositoryRefreshCancel;
	std::future<GeneralsArsenalLauncher::ModificationOperationResult> modificationOperation;
	bool modificationOperationRunning = false;
	std::shared_ptr<GeneralsArsenalLauncher::ModificationOperationProgress> modificationOperationProgress;
	std::optional<GeneralsArsenalLauncher::ModificationType> modificationOperationType;
	std::string modificationOperationEngine;
	std::optional<GeneralsArsenalLauncher::ModificationStack> pendingModificationLaunch;
	...
	std::vector<GeneralsArsenalLauncher::RecoverableModification> recoverableModifications =
		GeneralsArsenalLauncher::ListRecoverableModifications(paths.mods);
	GeneralsArsenalLauncher::LocalImportRequest activeImportRequest;
	Uint64 transitionEpoch = SDL_GetTicks();
	Uint64 modificationUpdateFlashEpoch = 0;
	...
	if (parsed.internalTestSupervisorRecovery && parsed.internalRecovered) {
		fprintf(stderr, "INFO: Generals: Arsenal supervisor recovery test completed\n");
		fflush(stderr);
		running = false;
	...
	// GeneralsArsenal @refactor Codex 13/08/2026 Keep SAGE button behavior while giving every launcher control one emerald palette.
	...
	Button modsRefresh{kModsLayout.refreshButton, Localized(russian, "REFRESH", "ОБНОВИТЬ")};
	...
	std::vector<Button *> modsButtons{&modsGeneralsTab, &modsZeroHourTab, &modsModsTab, &modsPatchesTab,
		...
		&modsRefresh, &modsImport, &modsImportFolder, &modsRemove, &modsRestore, &modsLaunch, &modsBack};
	...
	// GeneralsArsenal @feature Codex 13/08/2026 Apply display settings only when the user explicitly changes them.
	auto applyLauncherWindowSettings = [&](const GeneralsArsenalLauncher::LauncherSettings &settings, bool centerWindow) -> bool {
		const bool fullscreen = !testWindowOverride &&
			settings.windowMode == GeneralsArsenalLauncher::LauncherWindowMode::Fullscreen;
	...
	auto updateLocalizedButtonLabels = [&]() {
		...
		modsRefresh.label = Localized(russian, "REFRESH", "ОБНОВИТЬ");
	...
	auto selectedModificationType = [&]() {
		switch (modsPage) {
			case ModsPage::Patches: return GeneralsArsenalLauncher::ModificationType::Patch;
			case ModsPage::Addons: return GeneralsArsenalLauncher::ModificationType::Addon;
			default: return GeneralsArsenalLauncher::ModificationType::Mod;
		}
	};
	auto currentModificationProfile = [&]() -> GeneralsArsenalLauncher::ModificationSelectionProfileV1 & {
		return modsEngine == kZeroHourProfileId ? zeroHourModificationProfile : generalsModificationProfile;
	};
	auto saveModificationProfile = [&](GeneralsArsenalLauncher::ModificationSelectionProfileV1 &profile) {
		std::string profileError;
		if (!GeneralsArsenalLauncher::SaveModificationSelectionProfile(paths.mods, profile, profileError)) {
	...
	auto reconcileModificationProfiles = [&]() {
		for (GeneralsArsenalLauncher::ModificationSelectionProfileV1 *profile :
			{&generalsModificationProfile, &zeroHourModificationProfile}) {
			std::string profileWarning;
			if (GeneralsArsenalLauncher::ReconcileModificationSelectionProfile(
	...
	auto parentMatchesActiveStack = [&](const std::string &parentId,
		GeneralsArsenalLauncher::ModificationType type) {
		if (parentId.empty() || type == GeneralsArsenalLauncher::ModificationType::Mod) return true;
		const auto &profile = currentModificationProfile();
		const auto *activeMod = GeneralsArsenalLauncher::FindModification(
			modificationCatalog, modsEngine, profile.modSelection);
		const auto *activePatch = GeneralsArsenalLauncher::FindModification(
			modificationCatalog, modsEngine, profile.patchSelection);
		return (activeMod && parentId == activeMod->id) ||
			(type == GeneralsArsenalLauncher::ModificationType::Addon && activePatch && parentId == activePatch->id);
	};
	auto visibleModifications = [&]() {
		std::vector<ModificationViewItem> items;
		const auto selectedType = selectedModificationType();
		for (const GeneralsArsenalLauncher::RepositoryModification &available : repositoryCatalog.modifications) {
			if (available.engine != modsEngine || available.type != selectedType ||
				!parentMatchesActiveStack(available.parentId, available.type)) continue;
			auto existing = std::find_if(items.begin(), items.end(), [&](const ModificationViewItem &item) {
				return ToLower(item.id()) == ToLower(available.id);
			});
			if (existing != items.end()) {
				if (!existing->available || GeneralsArsenalLauncher::CompareModificationVersions(
					existing->available->version, available.version) < 0) {
					existing->available = &available;
				}
				continue;
			}
			ModificationViewItem item;
			item.available = &available;
			items.push_back(item);
		}
		for (const GeneralsArsenalLauncher::InstalledModification &installed : modificationCatalog.modifications) {
			...
			if (represented == items.end()) {
				items.push_back({&installed, nullptr});
				continue;
			}
			const std::string activeSelection = selectedType == GeneralsArsenalLauncher::ModificationType::Mod ?
				currentModificationProfile().modSelection :
				(selectedType == GeneralsArsenalLauncher::ModificationType::Patch ?
					currentModificationProfile().patchSelection : std::string{});
			const bool candidateIsActive = GeneralsArsenalLauncher::ModificationSelectionKey(installed) == activeSelection ||
				(selectedType == GeneralsArsenalLauncher::ModificationType::Addon &&
					GeneralsArsenalLauncher::IsModificationSelected(currentModificationProfile(), installed));
			const bool currentIsActive = represented->installed &&
				GeneralsArsenalLauncher::IsModificationSelected(currentModificationProfile(), *represented->installed);
			const bool exactAvailable = represented->available &&
				ToLower(installed.version) == ToLower(represented->available->version);
			if (!represented->installed || candidateIsActive || (!currentIsActive && exactAvailable) ||
				(!currentIsActive && !represented->available &&
					GeneralsArsenalLauncher::CompareModificationVersions(
						represented->installed->version, installed.version) < 0)) {
		...
		std::sort(items.begin(), items.end(), [&](const ModificationViewItem &left, const ModificationViewItem &right) {
			if (selectedType == GeneralsArsenalLauncher::ModificationType::Addon) {
				auto activeOrder = [&](const ModificationViewItem &item) {
					...
					const auto found = std::find(addons.begin(), addons.end(),
						GeneralsArsenalLauncher::ModificationSelectionKey(*item.installed));
			...
			return GeneralsArsenalLauncher::CompareModificationVersions(left.version(), right.version()) < 0;
		});
		return items;
	};
	auto repositoryHasUpdates = [&]() {
		return std::any_of(repositoryCatalog.modifications.begin(), repositoryCatalog.modifications.end(),
			[&](const GeneralsArsenalLauncher::RepositoryModification &available) {
				return std::any_of(modificationCatalog.modifications.begin(), modificationCatalog.modifications.end(),
					[&](const GeneralsArsenalLauncher::InstalledModification &installed) {
						return installed.engine == available.engine && installed.type == available.type &&
							installed.id == available.id && GeneralsArsenalLauncher::CompareModificationVersions(
								installed.version, available.version) < 0;
					});
			});
	};
	auto installedVersions = [&](const ModificationViewItem *item) {
		std::vector<const GeneralsArsenalLauncher::InstalledModification *> versions;
	...
	auto modificationContextLinks = [&]() {
		...
		if (selected->available) {
			links[0] = selected->available->modDbUrl.empty() ?
				selected->available->descriptorUrl : selected->available->modDbUrl;
			links[1] = selected->available->discordUrl;
			links[2] = selected->available->newsUrl;
			links[3] = selected->available->supportUrl;
		}
		if (links[0].empty() && selected->installed) links[0] = selected->installed->source;
	...
	auto reloadModificationCatalog = [&]() {
		modificationCatalog = GeneralsArsenalLauncher::LoadModificationCatalog(paths.mods);
		recoverableModifications = GeneralsArsenalLauncher::ListRecoverableModifications(paths.mods);
	...
	auto startRepositoryRefresh = [&](bool force) {
		if (repositoryRefreshRunning) return;
		repositoryRefreshRunning = true;
		statusMessage = Localized(russian, "UPDATING CATALOG IN BACKGROUND...", "КАТАЛОГ ОБНОВЛЯЕТСЯ В ФОНЕ...");
		statusError = false;
		const fs::path cacheRoot = paths.mods / "Cache";
		repositoryRefreshCancel = std::make_shared<std::atomic_bool>(false);
		const std::shared_ptr<std::atomic_bool> cancel = repositoryRefreshCancel;
		repositoryRefresh = std::async(std::launch::async, [cacheRoot, force, cancel]() {
			return GeneralsArsenalLauncher::RefreshRepositoryCatalog(cacheRoot, force, cancel.get());
		});
	};
	auto startNextModificationImport = [&]() {
		...
		const auto *activePatch = GeneralsArsenalLauncher::FindModification(
			modificationCatalog, modsEngine, profile.patchSelection);
		if (activeImportRequest.type == GeneralsArsenalLauncher::ModificationType::Patch && activeMod) {
			activeImportRequest.parentId = activeMod->id;
		} else if (activeImportRequest.type == GeneralsArsenalLauncher::ModificationType::Addon) {
		...
		modificationOperationProgress = std::make_shared<GeneralsArsenalLauncher::ModificationOperationProgress>();
		...
		modificationOperation = std::async(std::launch::async, [request, progress]() {
			return GeneralsArsenalLauncher::ImportLocalModification(request, progress.get());
		});
	};
	auto startRepositoryInstall = [&](const GeneralsArsenalLauncher::RepositoryModification &modification) {
		if (modificationOperationRunning) return;
		modificationOperationProgress = std::make_shared<GeneralsArsenalLauncher::ModificationOperationProgress>();
		const auto progress = modificationOperationProgress;
		modificationOperationRunning = true;
		modificationOperationType = modification.type;
		modificationOperationEngine = modification.engine;
		statusMessage = Localized(russian, "DOWNLOADING: ", "ЗАГРУЗКА: ") + modification.name;
		statusError = false;
		modificationOperation = std::async(std::launch::async, [modification, modsRoot = paths.mods, progress]() {
			return GeneralsArsenalLauncher::DownloadAndInstallModification(modification, modsRoot, progress.get());
		});
	};
	auto openMods = [&]() {
		reloadModificationCatalog();
		repositoryCatalog = GeneralsArsenalLauncher::LoadCachedRepositoryCatalog(paths.mods / "Cache");
		if (repositoryHasUpdates()) modificationUpdateFlashEpoch = SDL_GetTicks();
		startRepositoryRefresh(false);
		modsOpen = true;
		settingsOpen = false;
		modsSelectedItem = 0;
		modsScroll = 0;
	...
	auto openSettings = [&]() {
		...
		modificationCatalog = GeneralsArsenalLauncher::LoadModificationCatalog(paths.mods);
		std::string optionsError;
		generalsOptionsDraft = GeneralsArsenalLauncher::LoadGameOptions(
		...
		optionsError.clear();
		zeroHourOptionsDraft = GeneralsArsenalLauncher::LoadGameOptions(
		...
		optionsError.clear();
		generalsSagePatchDraft = GeneralsArsenalLauncher::LoadSagePatchOptions(
		...
		optionsError.clear();
		zeroHourSagePatchDraft = GeneralsArsenalLauncher::LoadSagePatchOptions(
	...
	auto saveSettings = [&]() -> bool {
		...
		if (!GeneralsArsenalLauncher::SaveSettingsBundle(paths.root, settingsDraft,
	...
	// GeneralsArsenal @tweak Codex 14/08/2026 Keep the launcher page limited to persistent launcher preferences.
	...
	auto currentLaunchSettings = [&]() -> GeneralsArsenalLauncher::ProfileLaunchSettings & {
		return settingsPage == SettingsPage::ZeroHour ? settingsDraft.zeroHour : settingsDraft.generals;
	};
	auto currentGameOptions = [&]() -> GeneralsArsenalLauncher::GameOptions & {
		return settingsPage == SettingsPage::ZeroHour ? zeroHourOptionsDraft : generalsOptionsDraft;
	};
	auto currentSagePatchOptions = [&]() -> GeneralsArsenalLauncher::SagePatchOptions & {
		return settingsPage == SettingsPage::ZeroHour ? zeroHourSagePatchDraft : generalsSagePatchDraft;
	};
	auto cycleResolution = [&](GeneralsArsenalLauncher::GameOptions &options, int direction) {
	...
	auto adjustSettingsRow = [&](int direction) {
		if (settingsPage == SettingsPage::Launcher) {
			if (settingsSelectedRow == 0) {
				...
				settingsDraft.language = static_cast<GeneralsArsenalLauncher::LanguageMode>(language);
			} else if (settingsSelectedRow == 1) {
				settingsDraft.windowMode = settingsDraft.windowMode == GeneralsArsenalLauncher::LauncherWindowMode::Windowed ?
					GeneralsArsenalLauncher::LauncherWindowMode::Fullscreen : GeneralsArsenalLauncher::LauncherWindowMode::Windowed;
	...
	auto applySettingsDefaults = [&]() {
		if (settingsPage == SettingsPage::Launcher) {
			settingsDraft.language = GeneralsArsenalLauncher::LanguageMode::System;
			settingsDraft.windowMode = GeneralsArsenalLauncher::LauncherWindowMode::Windowed;
	...
	auto selectCompatibleModification = [&](GeneralsArsenalLauncher::ModificationSelectionProfileV1 &profile,
		const GeneralsArsenalLauncher::InstalledModification *modification, std::string &errorMessage) {
		return GeneralsArsenalLauncher::SelectCompatibleModification(
	...
	auto activateSelectedModificationCard = [&]() {
		...
		GeneralsArsenalLauncher::ReconcileModificationSelectionProfile(
	...
	auto activateModsButton = [&](Button *button) {
		...
		if (button == &modsContextClose) {
			...
			if (!items.empty()) {
				...
				if (selected.installed && !modificationOperationRunning) {
					const GeneralsArsenalLauncher::InstalledModification modification = *selected.installed;
					modificationOperationProgress = std::make_shared<GeneralsArsenalLauncher::ModificationOperationProgress>();
					const auto progress = modificationOperationProgress;
					modificationOperationRunning = true;
					...
					modificationOperation = std::async(std::launch::async, [modification, progress]() {
						GeneralsArsenalLauncher::ModificationOperationResult result;
						result.success = GeneralsArsenalLauncher::VerifyInstalledModification(
			...
			if (!items.empty()) {
				...
				if (selected.installed) {
					...
					modificationCoverTargetSelection = GeneralsArsenalLauncher::ModificationSelectionKey(*selected.installed);
		...
		else if (button == &modsRefresh) {
			reloadModificationCatalog();
			startRepositoryRefresh(true);
			...
			if (!items.empty()) {
				const auto &selected = items[static_cast<size_t>(
					std::clamp(modsSelectedItem, 0, static_cast<int>(items.size()) - 1))];
				if (selected.installed) {
					const auto result = GeneralsArsenalLauncher::MoveInstalledModificationToTrash(paths.mods, *selected.installed);
			...
			if (!recoverableModifications.empty()) {
				const auto result = GeneralsArsenalLauncher::RestoreModificationFromTrash(
			...
			if (!items.empty()) {
				const auto &selected = items[static_cast<size_t>(
					std::clamp(modsSelectedItem, 0, static_cast<int>(items.size()) - 1))];
				if (selected.available && (!selected.installed || selected.hasUpdate())) {
					startRepositoryInstall(*selected.available);
				} else {
					GeneralsArsenalLauncher::ModificationStack stack;
					std::string stackError;
					if (GeneralsArsenalLauncher::BuildSelectedModificationStack(
	...
	auto settingsLanguageValue = [&]() {
		switch (settingsDraft.language) {
			case GeneralsArsenalLauncher::LanguageMode::English: return Localized(russian, "ENGLISH", "АНГЛИЙСКИЙ");
			case GeneralsArsenalLauncher::LanguageMode::Russian: return Localized(russian, "RUSSIAN", "РУССКИЙ");
	...
	auto settingsRowLabelValue = [&](int row) -> std::pair<std::string, std::string> {
		if (settingsPage == SettingsPage::Launcher) {
			...
			if (row == 1) return {Localized(russian, "Display mode", "Режим экрана"),
				settingsDraft.windowMode == GeneralsArsenalLauncher::LauncherWindowMode::Fullscreen ?
	...
	// GeneralsArsenal @feature Codex 14/08/2026 Restore the exact launcher presentation only after DXVK is fully quiescent.
	auto restoreLauncherWindow = [&](const SharedWindowState &launcherWindowState) -> bool {
		// GeneralsArsenal @bugfix Codex 13/08/2026 Commit DXVK destruction before changing window state or attaching a software surface.
		if (!SDL_SyncWindow(window)) {
	...
	auto launchProfile = [&](LauncherProfile &profile, const GeneralsArsenalLauncher::ModificationStack *contentStack) {
		uiAudio.reset();
		modCoverCache.reset();
		backdrop.reset();
		logo.reset();
		...
		if (!SDL_SyncWindow(window)) {
			...
			fflush(stderr);
			// GeneralsArsenal @bugfix Codex 13/08/2026 Do not touch a potentially poisoned graphics connection during cleanup.
		...
		const GeneralsArsenalEngineResultV2 result = RunProfile(profile, window, parsed.engineArguments,
		...
		fprintf(stderr, "INFO: Generals: Arsenal engine session result: %u\n", static_cast<unsigned int>(result));
		fflush(stderr);
		if (result == GENERALS_ARSENAL_ENGINE_EXIT_APPLICATION) {
			running = false;
			return;
		}
		if ((quiescenceFlags & GENERALS_ARSENAL_ENGINE_REQUIRED_QUIESCENCE_FLAGS) !=
		...
		if (result == GENERALS_ARSENAL_ENGINE_FATAL_ERROR) {
		...
		SDL_SetWindowTitle(window, GeneralsArsenalBrand::kProductName);
		if (!createRenderer()) {
	...
	if (parsed.internalTestCycles > 0) {
		if (parsed.internalTestReturnAfterUpdates == 0) {
			...
			for (uint32_t cycle = 0; cycle < parsed.internalTestCycles && running; ++cycle) {
				...
				fprintf(stderr, "INFO: Generals: Arsenal lifecycle test cycle %u/%u profile=%s\n",
			...
			if (running) {
				fprintf(stderr, "INFO: Generals: Arsenal lifecycle test completed %u cycles\n", parsed.internalTestCycles);
				fflush(stderr);
				running = false;
			}
		}
	...
	while (running) {
		...
		if (repositoryRefreshRunning &&
			repositoryRefresh.wait_for(std::chrono::milliseconds(0)) == std::future_status::ready) {
			try {
				repositoryCatalog = repositoryRefresh.get();
				repositoryRefreshRunning = false;
				repositoryRefreshCancel.reset();
				modCoverCache.reset();
				clampModsSelection();
				if (repositoryHasUpdates()) modificationUpdateFlashEpoch = SDL_GetTicks();
				statusMessage = Localized(russian, "CATALOG READY: ", "КАТАЛОГ ГОТОВ: ") +
					std::to_string(repositoryCatalog.modifications.size());
				statusError = false;
				if (!repositoryCatalog.warnings.empty()) {
					fprintf(stderr, "WARNING: Repository refresh completed with %zu recoverable errors; cached data remains available\n",
						repositoryCatalog.warnings.size());
					fflush(stderr);
				}
				// GeneralsArsenal @test Codex 15/08/2026 Exercise the real repository-to-install path without desktop input injection.
				if (!parsed.internalTestInstallModification.empty() && !internalTestInstallStarted) {
					const auto requested = std::find_if(repositoryCatalog.modifications.begin(),
						repositoryCatalog.modifications.end(), [&](const auto &modification) {
							return modification.engine == modsEngine &&
								ToLower(modification.id) == parsed.internalTestInstallModification;
						});
					internalTestInstallStarted = true;
					if (requested == repositoryCatalog.modifications.end()) {
						statusMessage = "Requested repository test modification was not found";
						statusError = true;
						internalTestInstallFinished = true;
						workerExitCode = 2;
					} else {
						startRepositoryInstall(*requested);
					}
				}
			} catch (const std::exception &exception) {
				repositoryRefreshRunning = false;
				repositoryRefreshCancel.reset();
				statusMessage = std::string("Repository refresh failed: ") + exception.what();
				statusError = true;
			}
		}
		if (modificationOperationRunning &&
			modificationOperation.wait_for(std::chrono::milliseconds(0)) == std::future_status::ready) {
			const GeneralsArsenalLauncher::ModificationOperationResult result = modificationOperation.get();
			...
			if (result.success) {
				...
				if (modificationOperationType && !result.selectionKey.empty()) {
					...
					if (const auto *installed = GeneralsArsenalLauncher::FindModification(
			...
			if (internalTestInstallStarted && !parsed.internalTestInstallModification.empty()) {
				internalTestInstallFinished = true;
				if (!result.success) workerExitCode = 2;
			}
		...
		if (const auto dialogResult = ConsumeModificationFileDialog(modificationFileDialog, selectedModificationFiles)) {
			if (!dialogResult->success) {
				...
				const auto *target = GeneralsArsenalLauncher::FindModification(modificationCatalog,
				...
				if (!target) {
					...
					const auto result = GeneralsArsenalLauncher::ReplaceModificationCover(
		...
		modsRefresh.enabled = modsRefresh.enabled && !repositoryRefreshRunning;
		...
		modsLink.enabled = !migrationRunning && !folderDialog.pending && !modificationFileDialog.pending &&
			!modificationOperationRunning && currentModItem &&
			((currentModItem->available && (!currentModItem->available->modDbUrl.empty() ||
				!currentModItem->available->discordUrl.empty() || !currentModItem->available->newsUrl.empty() ||
				!currentModItem->available->supportUrl.empty() || !currentModItem->available->descriptorUrl.empty())) ||
				(currentModItem->installed && currentModItem->installed->source.rfind("https://", 0) == 0));
		...
		const bool selectedNeedsInstall = currentModItem && currentModItem->available &&
			(!currentModItem->installed || currentModItem->hasUpdate());
		modsLaunch.label = selectedNeedsInstall ?
			(currentModItem->hasUpdate() ? Localized(russian, "UPDATE", "ОБНОВИТЬ") :
				Localized(russian, "INSTALL", "УСТАНОВИТЬ")) : Localized(russian, "LAUNCH", "ЗАПУСТИТЬ");
		const LauncherProfile *modsProfile = FindProfile(profiles, modsEngine);
		GeneralsArsenalLauncher::ModificationStack activeStack;
		std::string activeStackError;
		const bool canLaunchActiveStack = modsProfile && modsProfile->enabled &&
			GeneralsArsenalLauncher::BuildSelectedModificationStack(
		...
		modsLaunch.enabled = !migrationRunning && !folderDialog.pending && !modificationFileDialog.pending &&
			!modificationOperationRunning && currentModItem && (selectedNeedsInstall || canLaunchActiveStack);
		if (!settingsOpen) {
			for (Button *button : settingsButtons) {
				button->hilited = false;
				button->selected = false;
					...
					if (modsPage == ModsPage::Addons && (event.key.mod & SDL_KMOD_SHIFT)) {
						...
						if (!items.empty()) {
							const auto &selected = items[static_cast<size_t>(std::clamp(
								modsSelectedItem, 0, static_cast<int>(items.size()) - 1))];
							if (selected.installed) {
								reordered = GeneralsArsenalLauncher::MoveSelectedAddon(currentModificationProfile(),
									GeneralsArsenalLauncher::ModificationSelectionKey(*selected.installed), direction);
					...
					if (modificationCardDragging && modsPage == ModsPage::Addons && targetIndex >= 0) {
						...
						if (sourceIndex < static_cast<int>(items.size()) && targetIndex < static_cast<int>(items.size())) {
							...
							if (source.installed && target.installed &&
								GeneralsArsenalLauncher::IsModificationSelected(currentModificationProfile(), *source.installed) &&
								GeneralsArsenalLauncher::IsModificationSelected(currentModificationProfile(), *target.installed)) {
								const auto &addons = currentModificationProfile().addonSelections;
								const auto targetPosition = std::find(addons.begin(), addons.end(),
									GeneralsArsenalLauncher::ModificationSelectionKey(*target.installed));
								if (targetPosition != addons.end() && GeneralsArsenalLauncher::MoveSelectedAddonTo(
									currentModificationProfile(), GeneralsArsenalLauncher::ModificationSelectionKey(*source.installed),
		...
		if (pendingModificationLaunch) {
			GeneralsArsenalLauncher::ModificationStack stack = std::move(*pendingModificationLaunch);
		...
		if (modsOpen) {
			...
			font.draw(Localized(russian, "ARSENAL MOD MANAGER", "МЕНЕДЖЕР МОДОВ ARSENAL"), 800.0f, 108.0f, 28.0f,
			...
			for (int visible = 0; visible < 4; ++visible) {
				...
				const bool active = item.installed && GeneralsArsenalLauncher::IsModificationSelected(
						currentModificationProfile(), *item.installed);
				int activeOrder = 0;
				if (active && item.installed && item.installed->type == GeneralsArsenalLauncher::ModificationType::Addon) {
					const auto &addons = currentModificationProfile().addonSelections;
				...
				const Uint64 flashElapsed = SDL_GetTicks() - modificationUpdateFlashEpoch;
				const bool updateFlash = item.hasUpdate() && modificationUpdateFlashEpoch != 0 &&
					flashElapsed < 5000 && (flashElapsed / 250) % 2 == 0;
				DrawModificationCard(renderer, font, modCoverCache, item, card,
					index == modsSelectedItem, active, activeOrder, russian, updateFlash);
			}
			if (items.empty()) {
				...
				const bool selectedActive = selected.installed && GeneralsArsenalLauncher::IsModificationSelected(
						currentModificationProfile(), *selected.installed);
				font.draw(selectedActive ? Localized(russian, "STATUS: ACTIVE", "СТАТУС: АКТИВЕН") :
					(selected.hasUpdate() ? Localized(russian, "STATUS: UPDATE AVAILABLE", "СТАТУС: ДОСТУПНО ОБНОВЛЕНИЕ") :
						(selected.installed ?
							Localized(russian, "STATUS: INSTALLED", "СТАТУС: УСТАНОВЛЕН") :
							Localized(russian, "STATUS: AVAILABLE", "СТАТУС: ДОСТУПЕН"))),
				...
				font.draw(Localized(russian, "CONTENT TYPE", "ТИП СОДЕРЖИМОГО") + std::string(": ") +
					GeneralsArsenalLauncher::ModificationTypeName(selected.type()),
			...
			// GeneralsArsenal @bugfix Codex 14/08/2026 Draw controls after their panel surfaces so detail actions remain visible and interactive.
			...
			DrawGradientRect(renderer, SDL_FRect{kSettingsLayout.panel.x, kSettingsLayout.panel.y, kSettingsLayout.panel.w, 3.0f},
				LauncherPalette::kAccent, LauncherPalette::kAccentBright);
			font.draw(Localized(russian, "ARSENAL SETTINGS", "НАСТРОЙКИ ARSENAL"), 800.0f, 145.0f, 30.0f,
				LauncherPalette::kTextBright, true);
		...
		// GeneralsArsenal @test Codex 13/08/2026 Capture the settled SAGE composition instead of its empty first transition frame.
		...
		// GeneralsArsenal @test Codex 15/08/2026 Let repository QA capture only after the asynchronous live catalog is applied.
		const bool repositoryReadyForScreenshot =
			(!parsed.internalTestWaitForRepository || !repositoryRefreshRunning) &&
			(parsed.internalTestInstallModification.empty() || internalTestInstallFinished);
		if (!internalScreenshotWritten && !parsed.internalTestScreenshot.empty() && screenshotCompositionReady &&
			repositoryReadyForScreenshot) {
	...
	if (repositoryRefreshRunning) {
		if (repositoryRefreshCancel) repositoryRefreshCancel->store(true, std::memory_order_relaxed);
		repositoryRefresh.wait();
	}
	if (modificationOperationRunning) {
		if (modificationOperationProgress) {
			modificationOperationProgress->cancelRequested.store(true, std::memory_order_relaxed);
