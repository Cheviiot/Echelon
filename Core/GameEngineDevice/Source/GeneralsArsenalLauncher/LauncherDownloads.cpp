...
{
	// GeneralsArsenal @test Codex 14/08/2026 Trust only an explicitly injected private CA in isolated download fixtures.
	if (const char *path = std::getenv("GENERALS_ARSENAL_QA_CA_BUNDLE"); path && path[0]) {
...
std::string NormalizeDownloadUrl(std::string url)
{
	if (url.rfind("https://www.dropbox.com/", 0) == 0 || url.rfind("https://dropbox.com/", 0) == 0) {
		// GeneralsArsenal @bugfix Codex 15/08/2026 Preserve Dropbox share keys while requesting the direct file response.
	...
	ModificationOperationProgress *progress)
{
	...
	curl_easy_setopt(curl, CURLOPT_USERAGENT, "Generals-Arsenal/1 ModificationInstallerV1");
	...
	std::atomic_uint64_t *aggregateCompleted = nullptr, bool allowFullRestart = true)
{
	...
	if (resumeOffset != 0 && responseCode == 200) {
		// GeneralsArsenal @bugfix Codex 15/08/2026 Restart safely when an origin ignores Range instead of requiring a second click.
...
// GeneralsArsenal @feature Codex 15/08/2026 Download immutable author-hosted file sets concurrently and resume each file independently.
