	...
	const std::unordered_map<std::string, std::string> &values, fs::path &publishedPath, std::string &warning)
{
	...
	// GeneralsArsenal @feature Codex 14/08/2026 Publish V1 metadata atomically while leaving the legacy file recoverable.
	output << "[Modification]\nSchemaVersion=2\n"
		<< "Id=local:" << legacyId << "\nSourceId=local\n"
...
// GeneralsArsenal @feature Codex 14/08/2026 Compare numeric version components naturally while keeping release builds newer than pre-releases.
	...
	const std::vector<std::string> &addonSelections, ModificationStack &stack, std::string &errorMessage)
{
	...
	// GeneralsArsenal @feature Codex 15/08/2026 Reject incomplete or conflicting managed stacks before the VFS sees any content.
	for (const InstalledModification *layer : stack.layers) {
