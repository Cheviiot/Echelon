...
bool Configure(const GeneralsArsenalContentLayerV1 *layers, uint32_t count, std::string &errorMessage)
{
	...
	if (!layers || count > GENERALS_ARSENAL_MAX_CONTENT_LAYERS) {
	...
	for (uint32_t index = 0; index < count; ++index) {
		const GeneralsArsenalContentLayerV1 &source = layers[index];
		if (source.struct_size < sizeof(GeneralsArsenalContentLayerV1) ||
			source.layer_type > GENERALS_ARSENAL_CONTENT_LAYER_ADDON ||
