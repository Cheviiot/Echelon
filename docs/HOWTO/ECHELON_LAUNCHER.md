...
The host and both modules use the versioned C ABI declared in `LauncherIntegration/EngineModuleAPI.h`. A module exports the stable V2 entry point and the additive V3 session entry point:
...
The ABI includes the shared SDL window, game arguments, game-data and user-data paths, selected profile, policy values and an ordered array of `EchelonContentLayerV1` records. Each record carries its type, source-qualified ID, version, canonical read-only root, priority, and content fingerprint. Loose files resolve from the highest layer first; BIG/GIB files throughout each layer mount in ascending layer priority with verified overwrite precedence. V3 owns an explicit `Create → Prepare → Start → Step → Stop → Quiescent → Destroy` lifecycle and receives paths and policy through the host payload; V2 remains the blocking compatibility path and uses the scoped environment bridge only where legacy code still requires it. The engine clears the overlay before reporting the content-layer quiescence bit and returning `ReturnToLauncher`, `ExitApplication`, or `FatalError`.

The launcher keeps window ownership and renderer-neutral lifecycle notifications in `PresentationService`. Engine display requests use a callback that changes window mode and size; the V3 host also exposes a presentation bridge with started/frame/stopping/quiescent/failed events. Raw Vulkan devices and swapchains are never exchanged between the launcher renderer and the engine renderer. Preparation and launch failures are logged as structured diagnostics with a stage, code, profile, workspace, affected path and retryability flag.
